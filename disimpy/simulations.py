...
@numba.jit(nopython=True)
def _fill_uniformly_circle(n, radius, seed=123):
...
@numba.jit(nopython=True)
def _fill_uniformly_sphere(n, radius, seed=123):
...
@numba.jit(nopython=True)
def _fill_uniformly_ellipsoid(n, a, b, c, seed=123):
...
def _initial_positions_cylinder(n_spins, radius, R_inv, seed=123):
    ...
    positions[:, 1:3] = _fill_uniformly_circle(n_spins, radius, seed)
...
def _initial_positions_ellipsoid(n_spins, a, b, c, R_inv, seed=123):
    """Calculate positions for spins in a cylinder"""
    positions = _fill_uniformly_ellipsoid(n_spins, a, b, c, seed)
    positions = np.matmul(R_inv, positions.T).T
    return positions
...
def _cuda_step_cylinder(positions, g_x, g_y, g_z, phases, rng_states, t, gamma,
    ...
    if idx >= MAX_ITER:
        step_l = np.nan
    _cuda_mat_mul(step, R_inv)
    _cuda_mat_mul(r0, R_inv)
    for i in range(3):
        positions[thread_id, i] = positions[thread_id, i] + step[i] * step_l
    for m in range(g_x.shape[0]):
        phases[m, thread_id] += (gamma * dt *
...
def simulation(n_spins, diffusivity, gradient, dt, substrate, seed=123,
    ...
    if substrate['type'] == 'free':
        ...
        radius = substrate['radius']
        positions = _fill_uniformly_sphere(n_spins, radius, seed)
        if trajectories:
            with open(trajectories, 'w') as f:
                [f.write(str(i) + ' ') for i in positions.ravel()]
                f.write('\n')
